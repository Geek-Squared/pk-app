  ...
  dueSurveys(uid: string, interventionId: string): Observable<DueSurvey[]> {
    ...
    return combineLatest([intervention$, chapters$, workbook$]).pipe(
      switchMap(([intervention, chapters, workbooks]) => {
        ...
        // Filter to the ones not yet answered for THIS intervention and point.
        return combineLatest(
          surveyIds.map((surveyId) =>
            this.hasResponded(surveyId, uid, interventionId, timepoint).pipe(
              map((answered) => (answered ? null : { surveyId, interventionId, timepoint }))
            )
          )
